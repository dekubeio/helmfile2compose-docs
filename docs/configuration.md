...
!!! note "Upgrading past helmfile2compose v3.4.0"
    Regenerating with a newer release changes a few things you can see: `env` now wins over `envFrom`; every `$` in container commands is escaped (a `${VAR}` meant for compose goes in `overrides:`); mounts with `items` move to `configmaps/<name>_<hash>/`; PVC `subPath` is honoured (data already at the volume root keeps the old mount, with a warning); an extension that fails to load stops the run (exit 1). A LoadBalancer Service publishes its `port` instead of its `nodePort`. fix-permissions now emulates `fsGroup` (group ownership, `g+rwX`, setgid dirs, `group_add`). A `server-ca` Secret missing from the manifests keeps its mount with a warning — provide `./secrets/<name>/ca.crt`. With the nginx or traefik rewriter loaded, classless Ingresses carrying their annotations now go to them instead of HAProxy. servicemonitor (new `job_name`, so a new `job` label; `namespaceSelector`) and fake-apiserver users: see the [full list](https://docs.dekube.io/reference/config/#upgrading-from-engine-v170).
